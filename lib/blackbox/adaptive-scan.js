...
export async function runAdaptiveProbes(target, opts = {}) {
  ...
  // Surface correlated attack chains the knowledge graph found (e.g. an auth
  // bypass that combines with an IDOR into cross-user access). This was computed
  // nowhere before; it's the highest-value output of the graph. Best-effort.
  try {
    for (const chain of graph.discoverChains()) {
      allFindings.push({
        severity: chain.severity,
        title: `Attack chain: ${chain.type}`,
    ...
    // Chain discovery is best-effort; never fail the scan over it.
  }
