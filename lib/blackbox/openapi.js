...
/**
 * Does an invalid-credential response actually look like a *successful* login?
 * A bare HTTP 200 is not enough — SPA/JSON login endpoints routinely return 200
 * with `{ "success": false }` or an error body. Only flag when there's a real
 * success signal (an auth cookie or a token/success body) and no failure signal.
 */
export function looksAuthenticated(res) {
  if (!res || res.status !== 200) return false;
  const body = (res.body || "").toLowerCase();
  const failure = /invalid|incorrect|unauthor|denied|forbidden|"success"\s*:\s*false|\berror\b|\bfail|wrong|not found|missing|require/;
  if (failure.test(body)) return false;
  const setCookie = (res.headers?.["set-cookie"] || "").toLowerCase();
  const authCookie = /(sess|token|auth|jwt|sid)[\w-]*=[^;\s]+/.test(setCookie);
  const successBody = /"?(access_token|refresh_token|\btoken\b|jwt|session)"?\s*[:=]|"success"\s*:\s*true|authenticated|welcome/.test(body);
  return authCookie || successBody;
}
...
export async function probeAuthEndpoints(baseUrl, opts = {}) {
  ...
  // A redirect (e.g. apex→www), 404, or 405 means there's no real POST login
  // handler at this path — don't run the arbitrary-creds / rate-limit / SQLi
  // checks against a non-endpoint (avoids a false "no rate limiting" on a 308).
  if (validAttempt.status === 404 || validAttempt.status === 405 ||
      (validAttempt.status >= 300 && validAttempt.status < 400)) {
    return findings;
  }

  if (looksAuthenticated(validAttempt)) {
    findings.push({
      severity: "critical",
      title: "Login accepts arbitrary credentials",
      description: "POST /api/login returned a successful, authenticated response (auth cookie or token) for invalid test credentials.",
      url: loginUrl,
