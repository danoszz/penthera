...
function combine(scanS, attestS) {
  ...
  // Nothing measured and nothing attested is not a failure, it is an unknown.
  // Reporting it as a gap told an entity that had simply not filled in the
  // self-assessment that it had failed ten measures, which is both wrong and
  // the opposite of proportionate.
  if (known.length === 0) return attestS === "na" ? "n/a" : "not-assessed";
...
export function buildReadiness(scanResult, opts = {}) {
  ...
  const summary = { met: 0, partial: 0, gap: 0, "not-assessed": 0, "n/a": 0 };
...
export function formatReadinessMarkdown(readiness) {
  ...
  L.push(`**Summary:** ${s.met} met · ${s.partial} partial · ${s.gap} gap · ${s["not-assessed"]} not assessed · ${s["n/a"]} n/a, across ${readiness.totalMeasures} Article 21 measures.`);
  L.push("");
  L.push("*Not assessed* means neither the scan nor your self-assessment covered that measure yet. It is an unknown, not a failure. Run the self-assessment to turn those into real answers.");
  L.push("");
