...
function combine(scanS, attestS) {
  ...
  if (known.length === 0) return (scanS === "na" || attestS === "na") ? "n/a" : "gap";
...
export function formatReadinessMarkdown(readiness) {
  ...
  if (j?.incidentDeadlines?.length) {
    ...
    L.push("");
    L.push(`Report significant incidents to **${j.authorities?.csirt || "your CSIRT"}** (supervisor: ${j.authorities?.supervisor || "—"}):`);
    L.push("");
