  ...
  for (let i = 0; i < wordlist.length; i += concurrency) {
    ...
    const results = await Promise.allSettled(
      batch.map(async (path) => {
        ...
        if (!res) return null;
        // Measure the actual body length rather than the Content-Length header,
        // which is often absent on chunked responses. This matches the unit the
        // calibration baseline uses (text.length) so the noise filter in
        // autoCalibrate compares like with like instead of header-vs-body.
