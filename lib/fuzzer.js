...
export async function fuzzEndpoint(baseUrl, path, validBody, opts = {}) {
  ...
  for (const variant of variants) {
    ...
    try {
      ...
      // (No prototype-pollution flag here: a single request returning 200
      // proves nothing, `payload.__proto__` is truthy for every object, so the
      // old check fired on any object payload. Confirming pollution needs a
      // follow-up probe that observes a polluted property; that lives in the
      // adaptive engine, not the one-shot fuzzer.)
      if (res.status === 500) {
