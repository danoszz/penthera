...
// ── Timing-based detection helpers ───────────────────────────────────────

const TIMING_DELAY_MS = 2500; // payload asks the backend to sleep ~3s
const TIMING_FLOOR_MS = 3000;
...
/**
 * Confirm a suspected time-based injection.
 *
 * A single slow response proves nothing: a GC pause, a cold serverless start,
 * or ordinary network jitter all look identical to an injected sleep. So a
 * candidate is re-sent a few times and only reported when the MEDIAN delay
 * holds, which jitter does not survive.
 *
 * @param {function(): Promise<{elapsed:number}>} send - re-issues the request
 * @param {number} baselineMs
 * @param {number} samples
 * @returns {Promise<{confirmed: boolean, medianMs: number, samples: number[]}>}
 */
export async function confirmTimingDelay(send, baselineMs, samples = 3) {
  const timings = [];
  for (let i = 0; i < samples; i++) {
    const res = await send();
    if (!res || !res.ok) return { confirmed: false, medianMs: 0, samples: timings };
    timings.push(res.elapsed);
    // Bail early if a repeat comes back fast; a real sleep is consistent.
    if (res.elapsed < baselineMs + TIMING_DELAY_MS) {
      return { confirmed: false, medianMs: median(timings), samples: timings };
    }
  }
  const m = median(timings);
  return {
    confirmed: m > baselineMs + TIMING_DELAY_MS && m > TIMING_FLOOR_MS,
    medianMs: m,
    samples: timings,
  };
}
...
export async function probeSqli(baseUrl, path, opts = {}) {
  ...
  for (const { payload, db } of timingPayloads) {
    const send = () => {
      if (method === "GET") {
        const sep = path.includes("?") ? "&" : "?";
        return safeFetch(`${baseUrl}${path}${sep}id=${encodeURIComponent(payload)}`, {
          method,
          timeout: 8_000,
        });
      }
    ...
    // A slow first response is only a candidate. Re-sample before reporting, so
    // jitter and cold starts don't get published as a high-severity finding.
    if (res.elapsed > baseline.elapsed + TIMING_DELAY_MS && res.elapsed > TIMING_FLOOR_MS) {
      const check = await confirmTimingDelay(send, baseline.elapsed);
      if (check.confirmed) {
        findings.push({
          severity: "high",
          confidence: "likely", // inferred from timing, not proven output
          title: `Possible blind SQL injection (time-based, ${db})`,
          description: `Payload: ${payload}. Delay held across ${check.samples.length} repeats (median ${check.medianMs}ms vs baseline ${baseline.elapsed}ms).`,
          url: `${baseUrl}${path}`,
          status: res.status,
          category: "sqli",
          source: "injection-probe",
...
/**
 * Build SSTI probes with a randomized arithmetic canary.
 *
 * A fixed "7*7 = 49" canary produces false positives: "49" occurs naturally in
 * prices, IDs, counts, and timestamps, so any page containing it looked like a
 * confirmed critical. Instead we multiply two random 4-digit numbers, giving a
 * 7-8 digit product that is highly unlikely to appear by chance, and only flag
 * when the product shows up while the raw payload and the literal expression do
 * not, i.e. the template engine actually evaluated the arithmetic.
 */
export function buildSstiProbes() {
  const a = 1000 + Math.floor(Math.random() * 9000);
  const b = 1000 + Math.floor(Math.random() * 9000);
  const expr = `${a}*${b}`;
  const product = String(a * b);
  const probes = [
    { payload: `{{${expr}}}`, engine: "Jinja2/Twig/Handlebars" },
    { payload: `\${${expr}}`, engine: "Freemarker/Velocity/EL" },
    { payload: `#{${expr}}`, engine: "Thymeleaf/Ruby ERB" },
    { payload: `<%= ${expr} %>`, engine: "ERB/EJS" },
    { payload: `\${{${expr}}}`, engine: "AngularJS/Vue" },
    { payload: `{{constructor.constructor('return ${expr}')()}}`, engine: "Handlebars sandbox escape" },
  ];
  return { probes, expr, product };
}

/** True only when the response shows the canary product but not the raw payload/expression. */
export function sstiEvaluated(body, payload, expr, product) {
...
export async function probeSsti(baseUrl, path, opts = {}) {
  ...
  for (const { payload, engine } of probes) {
    ...
    if (res.ok && sstiEvaluated(res.body, payload, expr, product)) {
      findings.push({
        severity: "critical",
        title: `Server-Side Template Injection (${engine})`,
        description: `Payload "${payload}" evaluated to "${product}" in response`,
        url: `${baseUrl}${path}`,
        status: res.status,
        category: "ssti",
        source: "injection-probe",
    ...
    if (postRes.ok && sstiEvaluated(postRes.body, payload, expr, product)) {
      findings.push({
        severity: "critical",
        title: `Server-Side Template Injection (${engine})`,
        description: `POST payload "${payload}" evaluated to "${product}"`,
        url: `${baseUrl}${path}`,
...
export async function probeOpenRedirect(baseUrl, path, opts = {}) {
  ...
  for (const param of redirectParams) {
    for (const payload of REDIRECT_PAYLOADS.slice(0, 3)) {
      const sep = path.includes("?") ? "&" : "?";
      const reqUrl = `${baseUrl}${path}${sep}${param}=${encodeURIComponent(payload)}`;
      const res = await safeFetch(reqUrl, { timeout: 5_000 });

      if (!res.ok) continue;

      // Only a real open redirect if the Location actually resolves to the
      // attacker host, not if "evil.com" merely rides along in a same-site
      // canonicalization redirect's query string.
      const location = res.headers.location || "";
      if (redirectsToHost(location, reqUrl, "evil.com")) {
        findings.push({
          severity: "medium",
...
export async function probeCmdi(baseUrl, path, opts = {}) {
  ...
  for (const { payload, timing, pattern } of CMDI_PAYLOADS) {
    const send = () => safeFetch(`${baseUrl}${path}`, {
      method,
      headers: { "Content-Type": "application/json" },
    ...
    // Same rule as blind SQLi: one slow response is a candidate, not a finding.
    if (timing && res.elapsed > baseline.elapsed + TIMING_DELAY_MS && res.elapsed > TIMING_FLOOR_MS) {
      const check = await confirmTimingDelay(send, baseline.elapsed);
      if (check.confirmed) {
        findings.push({
          severity: "high",
...
/**
 * Per-probe reflection canary.
 *
 * A fixed string is trivially WAF-fingerprintable and can collide with content
 * left behind by an earlier scan, so each run gets a fresh one. It stays
 * alphanumeric and lowercase so no context (attribute, JS string, URL) mangles
 * or encodes it on the way through.
 */
export function makeCanary() {
  return "pnth" + Math.random().toString(36).slice(2, 10) + "xss";
}

/**
 * Reflected content is only executable as XSS when the browser renders the
 * response as HTML. A JSON/text API that echoes the payload is not XSS, so we
 * require an explicit HTML content type before confirming a reflection.
 */
...
export async function probeXss(baseUrl, path, opts = {}) {
  ...
  for (const param of inputParams) {
    ...
    if (!canaryRes.ok || !canaryRes.body.includes(CANARY) || !isHtmlResponse(canaryRes)) continue;
  ...
  if (postRes.ok && postRes.body.includes(CANARY) && isHtmlResponse(postRes)) {
