...
/**
 * Verify a binary's code signature. Returns true only when codesign exits 0.
 *
 * Uses execFileSync so the path is an argv entry: these paths are read from
 * plists that any local process can write, and interpolating one into a shell
 * string would hand a malicious persistence entry command execution inside the
 * scanner. Anything other than a clean exit counts as unsigned.
 */
function verifySignature(program) {
  try {
    execFileSync("codesign", ["-v", "--", program], {
      encoding: "utf-8",
      timeout: 15_000,
      stdio: ["pipe", "pipe", "pipe"],
    });
...
function checkXProtect() {
  ...
  // XProtect ships enabled on modern macOS, so its presence isn't interesting.
  // Pending signature updates are: they mean known malware may go undetected.
  const updatesPending = /updates available/i.test(status);
  return {
    name: "XProtect (Apple Malware Detection)",
    status: version || "unknown",
    detail: status,
    finding: updatesPending
      ? {
          severity: "low",
          title: "XProtect signature updates pending",
          description: "macOS reports XProtect updates are available. Until they install, recently known malware may not be detected.",
          category: "machine",
          source: "machine-audit",
        }
      : null,
...
function checkPersistence() {
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      // `program` comes from an attacker-writable plist, so it is passed as an
      // argv entry, never interpolated into a shell string. Verification uses
      // codesign's exit status rather than string-matching its output, so an
      // unexpected message can't be read as "signed" (fail closed, not open).
      let signed = null;
      if (program !== "unknown" && existsSync(program)) {
        signed = verifySignature(program);
      }
