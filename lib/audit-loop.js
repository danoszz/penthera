...
export function fingerprintFinding(f) {
  // Collapse only path segments that are entirely numeric (IDOR object ids like
  // /users/1 → /users/#), keeping version-like segments (v1, v2) distinct.
  const normUrl = (u) => String(u || "").toLowerCase().trim()
    .split("/").map((seg) => (/^\d+$/.test(seg) ? "#" : seg)).join("/");
  // Keep digits in the title: versions, ciphers, CVEs, protocols, and IP:port
  // pairs are exactly what tell otherwise-similar findings apart.
  const normTitle = (t) => String(t || "").toLowerCase().replace(/\s+/g, " ").trim();
  // Dependency-CVE findings can share one title (the CVE lives in the
  // description), so fold in a short description discriminator for those.
  const disc = (f.category === "cve" || f.category === "js-vulnerability")
    ? "|" + String(f.description || "").toLowerCase().replace(/\s+/g, " ").slice(0, 80)
    : "";
  return [f.source || "", f.category || "", normUrl(f.url), normTitle(f.title)].join("|") + disc;
}
...
export function updateHistory(history, findings, now) {
  // Deep-copy records so we never mutate the caller's history object.
  const h = {
    version: 1,
    findings: Object.fromEntries(
      Object.entries(history?.findings || {}).map(([k, v]) => [k, { ...v }]),
    ),
    scans: [...(history?.scans || [])],
  };
