...
export const PROBE_WSTG = [
  { id: "reachability", probe: "Reachability & fingerprint", wstg: ["WSTG-INFO-02", "WSTG-INFO-04"], profiles: ["quick", "standard", "deep"], modes: ["url"] },
  { id: "endpoint-discovery", probe: "Endpoint discovery", wstg: ["WSTG-INFO-06", "WSTG-INFO-07"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "tls", probe: "TLS / certificate audit", wstg: ["WSTG-CRYP-01"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "security-headers", probe: "Security headers", wstg: ["WSTG-CONF-12", "WSTG-CONF-14"], profiles: ["quick", "standard", "deep"], modes: ["url"] },
  { id: "openapi", probe: "OpenAPI / Swagger exposure", wstg: ["WSTG-INFO-09", "WSTG-CONF-02"], profiles: ["quick", "standard", "deep"], modes: ["url"] },
  { id: "sensitive-files", probe: "Sensitive file templates", wstg: ["WSTG-CONF-02", "WSTG-CONF-04"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "cors", probe: "CORS validation", wstg: ["WSTG-CLNT-07"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "cookie", probe: "Cookie security", wstg: ["WSTG-SESS-02"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "auth-hardening", probe: "Auth endpoint hardening", wstg: ["WSTG-ATHN-03", "WSTG-ATHN-10"], profiles: ["quick", "standard", "deep"], modes: ["url"] },
  { id: "client-auth", probe: "Client-side-only auth detection", wstg: ["WSTG-ATHZ-02", "WSTG-CLNT-01"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "jwt", probe: "JWT probes", wstg: ["WSTG-ATHN-04", "WSTG-SESS-10"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "idor", probe: "IDOR / BOLA", wstg: ["WSTG-ATHZ-04", "WSTG-ATHZ-05"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "oauth", probe: "OAuth open redirect", wstg: ["WSTG-ATHN-11", "WSTG-CLNT-04"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "adaptive", probe: "Adaptive escalation probes", wstg: ["WSTG-ATHZ-02", "WSTG-ATHZ-04", "WSTG-CLNT-07"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "retirejs", probe: "Retire.js (JS CVEs)", wstg: ["WSTG-CONF-04", "WSTG-CLNT-02"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "param-discovery", probe: "Parameter discovery", wstg: ["WSTG-INFO-06"], profiles: ["standard", "deep"], modes: ["url"] },
  { id: "recon", probe: "OSINT recon", wstg: ["WSTG-INFO-01", "WSTG-INFO-07"], profiles: ["deep"], modes: ["url"] },
  { id: "sqli", probe: "SQL injection", wstg: ["WSTG-INPV-05"], profiles: ["deep"], modes: ["url"] },
  { id: "ssti", probe: "SSTI", wstg: ["WSTG-INPV-18"], profiles: ["deep"], modes: ["url"] },
  { id: "ssrf", probe: "SSRF", wstg: ["WSTG-INPV-19"], profiles: ["deep"], modes: ["url"] },
  { id: "xss", probe: "Reflected XSS", wstg: ["WSTG-INPV-01", "WSTG-CLNT-01"], profiles: ["deep"], modes: ["url"] },
  { id: "cmdi", probe: "Command injection", wstg: ["WSTG-INPV-12"], profiles: ["deep"], modes: ["url"] },
  { id: "open-redirect", probe: "Open redirect (injection suite)", wstg: ["WSTG-CLNT-04"], profiles: ["deep"], modes: ["url"] },
  { id: "api-fuzzing", probe: "API fuzzing", wstg: ["WSTG-INPV-11", "WSTG-BUSL-05"], profiles: ["deep"], modes: ["url"] },
  { id: "secret-scanning", probe: "Secret scanning", wstg: ["WSTG-CONF-02", "WSTG-CONF-05"], profiles: ["quick", "standard", "deep"], modes: ["repo"] },
  { id: "trust-boundary", probe: "Route / trust-boundary analysis", wstg: ["WSTG-ATHZ-02", "WSTG-CONF-02"], profiles: ["quick", "standard", "deep"], modes: ["repo"] },
  { id: "risky-patterns", probe: "Risky code patterns", wstg: ["WSTG-INPV-01", "WSTG-INPV-12"], profiles: ["standard", "deep"], modes: ["repo"] },
];
...
export function getProbesForScan(result, profile = "standard") {
  // When the scan recorded which probes actually executed, trust that, the
  // coverage report must never claim a probe ran when it didn't (e.g. injection
  // probes that early-returned because no endpoints were discovered, or probes
  // skipped by profile). Fall back to the profile/mode catalog only when no
  // execution record is present (older callers, unit tests).
  const executed = Array.isArray(result?.executedProbes) ? new Set(result.executedProbes) : null;
  if (executed) {
    return PROBE_WSTG.filter((p) => executed.has(p.id));
  }
